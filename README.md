...
If the target file name is briefly locked (for example a same-named file was just deleted or is still syncing, in either OneDrive or a channel's SharePoint library), the upload is retried automatically under a de-duplicated name like `report (2).xlsx`, so read the final name from the returned item rather than assuming the original.
